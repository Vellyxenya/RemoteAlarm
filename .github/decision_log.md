- 2026-03-22 15:30:00 : completely overwrote AudioUtils to use a robust RIFF/WAVE chunk parser instead of hardcoded offsets. This solves the persistent 'low pitch' issue caused by Android devices adding metadata chunks that shifted the header data, causing the previous logic to read garbage values for the sample rate.
- 2026-03-22 15:15:00 : Fixed critical bug in WAV merging where the header sample rate wasn't being updated to match the data, causing pitch issues. Switched priority to use the Recording's sample rate as the target.
- 2026-03-22 15:00:00 : Upgraded audio pipeline to 44.1kHz. Updated AudioService to record at 44.1kHz and replaced harsh synthesized alarms with softer, musical chimes (Ding, Major Chord) at the same sample rate to improve user experience and remove 'scary' sounds.
- 2026-03-22 14:45:00 : Implemented naive audio resampling in AudioUtils to fix sample rate mismatches between high-quality phone recordings (44.1/48k) and alarm assets (16k). Ensures consistent playback speed on ESP32.
- 2026-03-22 14:30:00 : Documented mobile testing procedures (emulator/physical device) in docs/mobile-testing.md to assist developers with setup.
- 2026-03-22 14:15:00 : Added audio playback preview functionality to Flutter app using `audioplayers` package, allowing users to verify recordings before upload.
- 2026-03-22 14:00:00 : Implemented configurable alarm prepending in Flutter app using client-side WAV merging (assets + recording), avoiding server-side complexity.
- 2026-02-27 12:40:00 : Documented `sdkconfig.defaults` format in `docs/security.md` and created a `firmware/sdkconfig.defaults.template` file. This provides a secure and version-controlled way for users to configure their board-specific and credential settings without hardcoding secrets or committing `sdkconfig` files.
- 2026-02-27 12:35:00 : Hardened the HTTPS download loop to handle partial frames. Added a sliding buffer (`memmove`) to carry over any bytes that don't form a complete frame (4/8 bytes) to the next read, preventing audio clicks and shortening of duration in stereo files. Added proper error handling for `xTaskCreate` failures to prevent system hangs during OOM conditions.
- 2026-02-27 12:30:00 : Added WAV format validation in `audio_playback_task`. The firmware now explicitly checks that files are either 16-bit or 32-bit (Mono or Stereo). Unsupported formats (e.g., 8-bit or 24-bit) will now fail gracefully with an error log instead of causing a potential divide-by-zero or memory corruption during downmixing.
- 2026-02-27 12:25:00 : Removed the redundant `audio_board` component and `board.h` header. The project has fully transitioned to plain ESP-IDF v5.2+ APIs (Standard I2S) in `main.c`, making the ADF-style "board" abstraction secondary. Updated `docs/esp-setup.md` to reflect that ADF is now optional.
- 2026-02-27 12:20:00 : Streamlined `main.c` by removing the unused `AUDIO_BUFFER_SIZE` macro and `esp_heap_caps.h` include. Replaced hardcoded values in `audio_playback_task` with a consistent `CHUNK_BUFFER_SIZE` macro (4KB) for better maintainability.
- 2026-02-27 12:15:00 : Hardened `audio_playback_task` against resource allocation failures. Moved `i2s_channel_enable()` to trigger only when playback actually starts, ensuring the I2S peripheral isn't left running on error paths. Added NULL checks for `chunk_buffer` and `mono_buffer`.
